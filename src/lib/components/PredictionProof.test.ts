...
import PredictionProof from './PredictionProof.svelte';
...
describe('PredictionProof', () => {
	it('shows a pending message before the proof job has run', () => {
		render(PredictionProof, { props: { item: makeItem() } });
	...
	it('shows the hash, download link, and verify command on success', () => {
		...
		const { container } = render(PredictionProof, { props: { item } });
	...
	it('copies the hash to the clipboard when the copy button is clicked', async () => {
		...
		render(PredictionProof, { props: { item } });
