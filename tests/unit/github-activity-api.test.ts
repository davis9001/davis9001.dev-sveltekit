...
describe('GitHub Activity API', () => {
	...
	beforeEach(() => {
		vi.resetModules();
		// Freeze the clock (Date only) at midday UTC: several tests compute
		// "today" via toISOString(), which disagrees with the route's local-date
		// window once local evening rolls past UTC midnight (date-boundary flake).
	...
	describe('GET /api/github-activity', () => {
		it('should return activity data as JSON array', async () => {
			...
			const html = createContributionHTML([{ date: dateStr, level: 2, count: 5 }]);
		...
		it('should parse contribution levels from HTML', async () => {
			...
			const html = createContributionHTML([{ date: dateStr, level: 3, count: 10 }]);
	...
	describe('GET with GH_API_TOKEN', () => {
		...
		it('should handle events API with timestamps', async () => {
			const today = new Date();
			const dateStr = today.toISOString().split('T')[0];
			const html = createContributionHTML([{ date: dateStr, level: 1, count: 2 }]);
		...
		it('should merge commit hours and event hours', async () => {
			const today = new Date();
			const dateStr = today.toISOString().split('T')[0];
			const html = createContributionHTML([{ date: dateStr, level: 3, count: 8 }]);
			...
			mockFetch.mockResolvedValueOnce({
				ok: true,
				json: () => Promise.resolve([{ commit: { author: { date: `${dateStr}T10:00:00Z` } } }])
			});

			// Events API returns events for same date
			mockFetch.mockResolvedValueOnce({
				ok: true,
				json: () => Promise.resolve([{ created_at: `${dateStr}T20:00:00Z`, type: 'PushEvent' }])
			});
		...
		it('should paginate through commits when more than 100 results', async () => {
			const today = new Date();
			const dateStr = today.toISOString().split('T')[0];
			const html = createContributionHTML([{ date: dateStr, level: 4, count: 120 }]);
		...
		it('should handle commits without author date using committer date', async () => {
			const today = new Date();
			const dateStr = today.toISOString().split('T')[0];
			const html = createContributionHTML([{ date: dateStr, level: 1, count: 1 }]);
		...
		it('should return pmRatio of -1 for days without time data', async () => {
			...
			const html = createContributionHTML([{ date: dateStr, level: 1, count: 3 }]);
	...
	describe('D1 caching behavior', () => {
		...
		it('should return cached data from D1 when fresh (< 5 minutes)', async () => {
			const cachedActivity = [{ date: '2025-03-16', count: 5, level: 2, pmRatio: 0.5 }];
		...
		it('should fetch from GitHub when cache is stale (> 5 minutes)', async () => {
			const staleData = [{ date: '2025-03-16', count: 1, level: 1, pmRatio: -1 }];
		...
		it('should write to D1 cache after a successful GitHub fetch', async () => {
			...
			const writeCalls = prepareCalls.filter((c: string[]) => c[0].includes('REPLACE'));
