...
describe('embedPlaceholderHtml', () => {
	it('builds a placeholder without props', () => {
		expect(embedPlaceholderHtml('dirac-sea')).toBe('<div data-svelte-embed="dirac-sea"></div>');
	});

	it('builds a placeholder with encoded props', () => {
		const html = embedPlaceholderHtml('dirac-sea', { height: 300 });
		expect(html).toBe(
			'<div data-svelte-embed="dirac-sea" data-props="{&quot;height&quot;:300}"></div>'
		);
	});
...
describe('parseEmbedProps', () => {
	it('parses entity-escaped JSON', () => {
		expect(parseEmbedProps('{&quot;height&quot;:300}')).toEqual({ height: 300 });
	});
...
describe('parseContentSegments', () => {
	...
	it('splits html around a single embed', () => {
		const html =
			'<p>before</p>' + embedPlaceholderHtml('dirac-sea', { height: 1 }) + '<p>after</p>';
		expect(parseContentSegments(html)).toEqual([
			{ type: 'html', html: '<p>before</p>' },
			{ type: 'embed', name: 'dirac-sea', props: { height: 1 } },
